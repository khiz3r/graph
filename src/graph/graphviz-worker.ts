...
export class VizWorker {
  ...
  async renderString(dot: string): Promise<string> {
    ...
    // Skip caching SVGs derived from large schemas — the compressed result
    // can still exceed localStorage quota (typically 5–10 MB) and throw a
    // DOMException that would crash the render pipeline entirely.
    const SVG_CACHE_BYTE_LIMIT = 3 * 1_048_576; // 3 MB uncompressed SVG
    const shouldCache = cacheKey != null && svg.length < SVG_CACHE_BYTE_LIMIT;
    ...
    if (shouldCache) {
      ...
      console.info(
        `graphql-voyager: Skipping cache for large SVG (${(svg.length / 1_048_576).toFixed(1)} MB)`,
      );
    }
    return svg;
  }
